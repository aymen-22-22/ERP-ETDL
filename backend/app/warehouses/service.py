...
async def set_warehouse_image(
    session: AsyncSession, tenant_id: UUID, warehouse_id: UUID, file: UploadFile
) -> Warehouse:
    warehouse = await get_warehouse_or_404(session, tenant_id, warehouse_id)
